...
<h2 id="5.4">段式管理</h2>
段式管理把一个进程的虚地址控件设计成二维结构，即段号s与段内地址w。与页式管理不一样的是，页式管理中，被划分的页号按顺序编号递增排列，属于一维空间，而段式管理中的段号与段号之间无顺序关系。段的长度是不固定的。

<h2 id="5.5">段页式管理</h2>
段页式管理时，一个进程仍然拥有一个自己的二维地址空间，这与段式端丽相同。首先，一个进程中所包含的具有独立逻辑功能的程序或数据仍被划分为段，并有各自的段号s。这反映和继承了段式管理的特征。其次，对于段s中的程序或数据，则按照一定的大小将其划分为不同的页。和页式系统一样，最后不足一页的部分仍占一页。这反映了段页式管理中的页式特征。
<center><img src="https://github.com/UMRhamster/Jobs2018/raw/master/operating%20system/segment_page_management.png"/></center>
<center>段页式管理中段表、页表与内存的关系</center>

<h1 id="6">六、文件系统</h1>
<h2 id="6.1">文件的逻辑结构与存取方法</h2> 

1. 逻辑结构
   * 字符流式的无结构文件
  
     对基本信息单位操作不多的文件较适于采用字符流的无结构方式。
   * 记录式的有结构文件
2. 存取方法
   * 顺序存取方法：按照文件的逻辑地址顺序存取。
   * 随机存取方法：允许用户根据记录的编号来存取文件的任一记录，或者是根据存取命令把读写指针移到欲读写处来读写。
   * 按关键字存取法：用于复杂的文件系统，特别是数据库管理系统中，文件的存取根据给定的关键字或记录名进行的。
<h2 id="6.2">文件的物理结构与存储设备</h2> 
...
1. 物理结构
   ...
   * 索引文件
     ...
     要求系统为每个文件建立一张索引表，表中每一栏目指出文件信息所在的逻辑块号和与之对应的物理块号。
2. 文件存储设备
   * 顺序存取设备：顺序存储设备只有在前面的物理块被存取访问过之后，才能存取后续的物理块的内容。
   * 直接存取设备：允许文件系统直接存取磁盘上的任意物理块。
...
### 调度算法举例
假设磁盘访问序列：98，183，37，122，14，124，65，67。读写头起始位置：53磁道处，进行磁盘调度。
* 先来先服务
...
* 中断控制方式
  ...
  CPU与设备之间有相应的中断请求线，而且在设备控制器的控制状态寄存器中有相应的中断允许位。
* DMA方式

  在外围设备和内存之间开辟直接的数据交换通路。
* 通道方式

  与DMA方式相类似，也是一种以内存为中心，实现设备和内存直接交换数据的控制方式，与DMA方式不同的是，控制都由专管输入输出的硬件——通道来进行控制。
<h2 id="7.2">中断</h2> 
中断是指计算机在执行期间，系统内发生任何非寻常的或非预期的急需处理事件，使得CPU暂时中断当前正在执行的程序而转去执行相应的事件处理程序，待处理完毕后又返回被中断处继续执行或调度新的进程执行的过程。

### 中断分类
* 外中断
  
  来自处理及和内存外部的中断，包括I/O设备发出的I/O中断、外部信号中断、各种定时器引起的时钟中断以及调试程序中设置的断电等引起的调式中断等。
* 内中断

  指在处理机和内存内部产生的中断。内中断一般称为陷阱。
### 中断处理过程
<center><img src="https://github.com/UMRhamster/Jobs2018/raw/master/operating%20system/int_process.png"/></center>
